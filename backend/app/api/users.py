...
from app.models.pydantic import User_Pydantic, UpdateUser, ResponseMessage, UpdateAddress, AddUserRole, DeleteUserRole
...
async def update_me(update_user: UpdateUser, current_active_user=Depends(get_current_active_user)
                    ) -> User_Pydantic:
    await current_active_user.update_from_dict(update_user.dict(exclude_unset=True)).save()
    return current_active_user


# Admin routes

@router.get('/', response_model=List[User_Pydantic], tags=["users", "admin"],
...
@router.get('/{user_id}', response_model=User_Pydantic, tags=["users", "admin"],
            dependencies=[Depends(RoleChecker(["admin"]))])
...
@router.put('/{user_id}', response_model=User_Pydantic, tags=["users", "admin"],
            dependencies=[Depends(RoleChecker(["admin"]))])
...
@router.put('/address/{user_id}', response_model=User_Pydantic, tags=["users", "admin"],
            dependencies=[Depends(RoleChecker(["admin"]))])
...
@router.post('/add-role/', response_model=User_Pydantic, tags=["users", "admin"],
            dependencies=[Depends(RoleChecker(["admin"]))])
...
@router.delete('/delete-role/', response_model=User_Pydantic, tags=["users", "admin"],
            dependencies=[Depends(RoleChecker(["admin"]))])
...
@router.delete('/{user_id}', tags=["users", "admin"], dependencies=[Depends(RoleChecker(["admin"]))],
...
async def delete_user(user_id: int):
    ...
    await user.delete()
    return ResponseMessage(detail=f"Gebruiker met email-adres {user.email} is verwijderd")
