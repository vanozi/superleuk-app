...
async def add_vakantie(vakantie: VakantieCreateSchema,
...
@router.post("/admin/add_vakantie_for_user", dependencies=[Depends(get_current_active_user), Depends(RoleChecker(["admin"]))],
             response_model=VakantiesResponseSchema)
async def add_vakantie_for_other_as_admin(vakantie: VakantieCreateSchemaForUserAsAdmin,
                                          current_active_user=Depends(get_current_active_user)):
    ...
    if user is None:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=(
                f"De user met id {vakantie.user_id} bestaat niet"
            ),
        )
    # alle vakanties van de user ophalen
