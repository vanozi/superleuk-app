  ...
  <UDashboardPanelContent class="p-0">
    <div v-for="(mail, index) in mails" :key="index" :ref="el => { mailsRefs[mail.id] = el as Element }">
      <div
        ...
        :class="[mail.unread ? 'text-gray-900 dark:text-white' : 'text-gray-600 dark:text-gray-300',
      ...
      >
        <div class="flex items-center justify-between" :class="[mail.unread && 'font-semibold']">
          ...
          <span>
            {{ isToday(new Date(mail.date)) ? format(new Date(mail.date), 'HH:mm') : format(new Date(mail.date), 'dd MMM') }}</span>
        </div>
