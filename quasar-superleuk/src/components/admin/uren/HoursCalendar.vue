...
import { ref, computed, Ref, provide, shallowRef } from 'vue';
...
async function handleFetchEvents(
  ...
  console.log(' vanuit handleFetchEvents');
  ...
  try {
    const response_data: IWorkingHours[] =
      await workinghoursStore.getAllBetweenDates(
    ...
    if (listWeekView.value == true) {
      const allDatesInWeek = getDatesBetweenTwoDatesAsString(
        fetchInfo.start,
        fetchInfo.end
      );
      const datesInResponse = extractDatesFromResponse(response_data);
      const datesNotInResponse = allDatesInWeek.filter(
        (date) => !datesInResponse.includes(date)
      );
      const workingHoursNotInResponse = datesNotInResponse.map((date) => {
        return {
          ...
          description: '',
          submitted:
            response_data.length > 0 ? response_data[0].submitted : false,
...
function handleDatesSet(dateInfo: DatesSetArg) {
  console.log('vanuit handleDatesSet ');
...
function openEditHoursDialog(eventInfo: EventImpl) {
  ...
  if (
    eventInfo._context.viewSpec.type == 'listWeek' &&
    eventInfo.extendedProps.submitted
  ) {
    return;
  }
