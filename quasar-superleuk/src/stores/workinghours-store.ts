...
export const useWorkingHours = defineStore('workinghours-store', () => {
  ...
  const milkingsInViewNotSubmittedComputed = computed(() => {
    let totalMilkings = 0;
    if (workingHoursBetweenDates.value) {
      for (const item of workingHoursBetweenDates.value) {
        if (!item.submitted) {
  ...
  const milkingsInViewSubmittedComputed = computed(() => {
    let totalMilkings = 0;
    if (workingHoursBetweenDates.value) {
      for (const item of workingHoursBetweenDates.value) {
        if (item.submitted) {
  ...
  async function getMyWeekOverview(from_date: string, to_date: string) {
    try {
      const response = await api.get('/working_hours/week_overview/', {
  ...
  return {
    ...
    milkingsInViewNotSubmittedComputed,
    milkingsInViewSubmittedComputed,
    getMyWeekOverview,
    myWeekOverView
